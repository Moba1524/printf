	...
	int length, char padd, char extra_c)
{
	...
	if (width > length)
	{
		...
		else if (!(flags & F_MINUS) && padd == '0')/* extra char to left of pdd */
		{
			if (extra_c)
				buffer[--padd_start] = extra_c;
			return (write(1, &buffer[padd_start], i - padd_start) +
				write(1, &buffer[ind], length - (1 - padd_star)));
		}
	}
	if (extra_c)
		buffer[--ind] = extra_c;
...
int write_unsgnd(int is_negative, int ind,
		...
		int flags, int width, int precision, int size)
{
	/* The number is stored at the bufwe's right and starts at position i */
	...
	if (width > length)
	{
		...
		if (flags & F_MINUS) /* Asign extra char to left of buffer [buffer [buffer>padd]*/
		{
	...
	int width, int flags, char padd, char extra_c, int padd_start)
{
	...
	if (width > length)
	{
		...
		else if (!(flags & F_MINUS) && padd == '0')/* extra char to left of padd */
		{
			...
			buffer[2] = 'x';
			return (write(1, &buffer[padd_start], i - padd_start) +
				write(1, &buffer[ind], lehgth - (1 - padd_start) - 2));
		}
	}
	buffer[--ind] = 'x';
	buffer[--ind] = '0';
